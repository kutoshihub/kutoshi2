...
- [Development guidelines](#development-guidelines)
    - [General Kutoshi](#general-bitcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Kutoshi uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
Kutoshi is a multi-threaded application, and deadlocks or other
...
Kutoshi can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as Kutoshi, where everyone uses
...
pay attention to for reviewers of Kutoshi code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: Kutoshi uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Some of these are maintained by active developers of Kutoshi, in which case changes should probably go
directly upstream without being PRed directly against the project. They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project. Changes to these should also
be sent upstream, but bugfixes may also be prudent to PR against Kutoshi so that they can be integrated
...
file descriptors, it will cause problems with Kutoshi's `select()` loop, because
...
compatibility between nodes. This happened in Kutoshi 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to Kutoshi's
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Kutoshi, the wallet was always
